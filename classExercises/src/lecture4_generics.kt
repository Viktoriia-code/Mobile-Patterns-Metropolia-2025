...
fun main() {
    ...
    if("abba".isPalindrome()) println("yes") else println("no")

    operator fun List<Int>.compareTo(list1: List<Int>): Int =
        if(this.all { it > list1.max() }) 1 else if(list1.all { it > this.max() }) -1 else 0

    println(listOf(3,2,3) > listOf(0,1,2))

    // val q4 = PriorityQueue<List<Int>>(capacity = 3)
}